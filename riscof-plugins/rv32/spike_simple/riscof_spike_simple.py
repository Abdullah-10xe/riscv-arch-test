...
#          cmd = self.compile_cmd.format(testentry['isa'].lower().replace('zicsr', ' ', 2), self.xlen, test, elf, compile_macros)
      ...
      # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' --misaligned --isa={0}_smepmp +signature={1} +signature-granularity=4 {2}'.format(self.isa, sig_file, elf)
            simcmd = simcmd + ';' + self.dut_exe + ' --isa={0}_smepmp --log-commits -l my.elf 2> {1}'.format(self.isa, log_file)
          else:
            simcmd = 'echo "NO RUN"'
